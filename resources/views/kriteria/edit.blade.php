                ...
                <form action="{{ route('kriteria.update', $kriteria->id_kriteria) }}" method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                ...
                                <select name="tipe_kriteria" class="form-control" required>
                                    <option value="1" {{ old('tipe_kriteria', $kriteria->tipe_kriteria) == 1 ? 'selected' : '' }}>Benefit (Semakin besar semakin baik)</option>
                                    <option value="0" {{ old('tipe_kriteria', $kriteria->tipe_kriteria) == 0 ? 'selected' : '' }}>Cost (Semakin kecil semakin baik)</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="form-group">
                                ...
                                <select name="tipe_input" id="tipe_input" class="form-control" required>
                                    <option value="pilihan" {{ old('tipe_input', $kriteria->tipe_input) == 'pilihan' ? 'selected' : '' }}>Pilihan (Dropdown)</option>
                                    <option value="manual" {{ old('tipe_input', $kriteria->tipe_input) == 'manual' ? 'selected' : '' }}>Manual (Ketik Angka)</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-2" id="satuan-wrapper">
                            <div class="form-group">
                                <label>Satuan</label>
                                <input type="text" name="satuan" class="form-control" placeholder="Contoh: %, petugas, orang" value="{{ old('satuan', $kriteria->satuan) }}">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="form-group row align-items-center">
                                ...
                                <div class="col-sm-2 min-col" style="display: none;">
                                    <input type="number" step="any" name="subs_min[{{ $nilai }}]" class="form-control" value="{{ old('subs_min.'.$nilai, $subs[$nilai]['minimal_nilai'] ?? '') }}" placeholder="Min">
                                    @error('subs_min.'.$nilai) <small class="text-danger d-block">{{ $message }}</small> @enderror
                                </div>
                                <div class="col-sm-2 max-col" style="display: none;">
                                    <input type="number" step="any" name="subs_max[{{ $nilai }}]" class="form-control" value="{{ old('subs_max.'.$nilai, $subs[$nilai]['maksimal_nilai'] ?? '') }}" placeholder="Max">
